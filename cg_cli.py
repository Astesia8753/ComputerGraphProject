...
if __name__ == '__main__':
    ...
    '''
    now_path=os.getcwd()
    input_file=now_path+'\\testCase\\testCase.txt'
    output_dir=now_path+'\\testCaseOut'
    os.makedirs(output_dir, exist_ok=True)
    ...
    with open(input_file, 'r') as fp:
        ...
        while line:
            ...
            if line[0] == 'resetCanvas':
                ...
                para_list.append(dx)
                para_list.append(dy)
                item_dict[item_id][1] = alg.translate(item_dict[item_id][1], para_list,'')
                ...
                para_list.append(y)
                para_list.append(r)
                item_dict[item_id][1] = alg.rotate(item_dict[item_id][1], para_list,'')
                item_dict[item_id][3] = np.array(pen_color)
            elif line[0] == 'scale':
                item_id = line[1]
                ...
                para_list.append(y)
                para_list.append(s)
                item_dict[item_id][1] = alg.scale(item_dict[item_id][1], para_list,'')
                item_dict[item_id][3] = np.array(pen_color)
            elif line[0] == 'clip':
                item_id = line[1]
                x0 = int(line[2])
                ...
                para_list.append(x1)
                para_list.append(y1)
                res = alg.clip(item_dict[item_id][1], para_list, algorithm)
                if len(res) == 0:
